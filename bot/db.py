...
class DB:
    ...
    async def connect(self):
        ...
        await self.pool.execute((Path(__file__).parents[1]/'db/schema.sql').read_text())
    ...
    async def join(self,rid,uid):
        await self.pool.execute('INSERT INTO room_members(room_id,user_id) VALUES($1,$2) ON CONFLICT (room_id,user_id) DO NOTHING',rid,uid)
