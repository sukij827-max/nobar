...
class DB:
    ...
    async def connect(self):
        ...
        try:
            ...
            print("[DB] PostgreSQL connection established", flush=True)
    ...
    async def user(self, u):
        await self.pool.execute(
            ...
            'ON CONFLICT(user_id) DO UPDATE SET username=$2,first_name=$3,updated_at=now()',
    ...
    async def room(self, code):
        return await self.pool.fetchrow(
            'SELECT * FROM rooms WHERE code=$1 AND is_active', code.upper()
        )

    async def make_room(self, code, gid, uid, title):
        return await self.pool.fetchrow(
            'INSERT INTO rooms(code,group_chat_id,host_user_id,title) VALUES($1,$2,$3,$4) RETURNING *',
            code, gid, uid, title
        )

    async def join(self, rid, uid):
        await self.pool.execute(
            'INSERT INTO room_members VALUES($1,$2) ON CONFLICT DO NOTHING', rid, uid
        )
