-- Nobar production PostgreSQL schema with idempotent legacy migration.
    ...
    FOR r IN
        ...
        WHERE contype='f'
          AND (
              ...
              OR conrelid IN ('users'::regclass,'films'::regclass,'rooms'::regclass,'room_members'::regclass)
          )
    LOOP
...
DELETE FROM users a USING users b WHERE a.user_id=b.user_id AND a.ctid>b.ctid;
...
ALTER TABLE films ADD COLUMN IF NOT EXISTS room_id BIGINT;
...
BEGIN
    ...
    WHERE a.code IS NOT NULL AND a.code=b.code AND a.ctid>b.ctid;
...
DELETE FROM room_members a USING room_members b
WHERE a.room_id=b.room_id AND a.user_id=b.user_id AND a.ctid>b.ctid;
...
CREATE INDEX IF NOT EXISTS films_room_status_idx ON films(room_id,status);
