...
def storage():
    return B2Storage(os.environ['B2_ENDPOINT'],os.environ['B2_BUCKET'],os.environ['B2_KEY_ID'],os.environ['B2_APPLICATION_KEY'],os.getenv('B2_REGION','us-east-005'))
...
async def upload_complete(code,body:dict,x_telegram_init_data: str=Header(default='')):
    ...
    try:
        ...
        try:
            ...
            etags=[str(p.get('ETag','')).strip() for p in parts]
        except (TypeError, ValueError):
            raise HTTPException(400,'Data part tidak valid')
        if len(parts)!=expected or numbers!=list(range(1,expected+1)) or any(not e for e in etags):
            raise HTTPException(400,'Part upload belum lengkap atau nomor part tidak valid')
        try:
